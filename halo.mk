...
# The following rule has as requirement *all* flowcell targets, but
# the input files to a given merge operation are the sample-specific
# bam files. Unfortunately, it does not seem possible to filter the
# flowcell targets based on a pattern in the prerequisite
%.sort.merge.bam: $(FLOWCELL_TARGETS)
	@$(eval INPUTFILES=$(addprefix INPUT=,$(filter $(dir $*)%, $(FLOWCELL_TARGETS))))
	$(PICARD_JAVA) $(PICARD_HOME)/MergeSamFiles.jar $(INPUTFILES) O=$@.tmp $(PICARD_OPTIONS_COMMON) $(PICARD_MERGESAM_OPTIONS) && mv $@.tmp $@ && mv $@.tmp.bai $(@:.bam=).bai
